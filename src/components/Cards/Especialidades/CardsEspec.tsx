import React from 'react'
import {Card, CardHeader, CardBody, Image} from "@nextui-org/react";

export const CardsEspec = () => {
  return (
    <section className='relative bg-[#F3F3F3]'>
        <div className='container flex flex-col-reverse lg:flex-row items center gap-12 py-20'>
            <div className='flex flex-1 flex-col items-center'>

            <div className="grid grid-cols-3 lg:grid-cols-5 gap-4 w-11/12">

                <a className='transition-all' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Gestion de las Organizaciones</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenGestion.jpg"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>
            

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Informatica</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenInfo.jpg"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Construcciones y Diseño</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenConstru.webp"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Química y Biotecnología</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenQuimica.jpg"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Producción Artística y Gestión Cultural</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenMusica.jpg"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Mecatronica</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenMeca.webp"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Diseño</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenDiseño.jpg"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Producción en Medios de Comunicación</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenMedios.jpg"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Tecnologías de la Información y las Comunicaciones</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenTIC.webp"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

                <a className='' href='#'>
                    <Card className="py-5 pb-10 hover:bg-gradient-to-r from-[#847EFC] to-[#352CE8] hover:text-white hover:transition-all border-solid border-2 border-[#c998e7]">
                        <CardHeader className="pb-5 pt-2 px-4 flex-col h-[90px]">
                            <h1 className="font-bold text-large text-center">Humanidades e Investigación Social</h1>
                        </CardHeader>
                        <CardBody className="overflow-visible py-2">
                            <Image
                            alt="Card background"
                            className="object-cover rounded-xl"
                            src= "/images/ImagenHuman.png"
                            style={{ width: '200px', height: '130px' }}
                            />
                        </CardBody>
                    </Card>
                </a>

            </div>

            </div>
        </div>

    </section>
  )
}
